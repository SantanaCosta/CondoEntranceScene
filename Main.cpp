...
void updateGate() {
    const float animationSpeed = 0.005f; // velocidade da animação em unidades por segundo
    const float maxHeight = 1.3f; // altura máxima do portão
    const float minHeight = 0.0f; // altura mínima do portão
    const float maxDeltaHeight = 0.01f; // limite superior para a variação da altura do portão

    static bool goingUp = true; // indica se o portão está subindo ou descendo
    
    // Obtém o tempo decorrido desde a última atualização da animação
    int elapsedTime = glutGet(GLUT_ELAPSED_TIME);
    
    // Calcula o deslocamento máximo permitido para a altura do portão com base no tempo decorrido e no limite superior da variação da altura
    float maxDeltaHeightAllowed = elapsedTime / 1000.0f * maxDeltaHeight;
    
    // Calcula a nova altura do portão com base no tempo decorrido e na velocidade da animação
    float deltaHeight = elapsedTime / 1000.0f * animationSpeed;
    deltaHeight = std::min(deltaHeight, maxDeltaHeightAllowed); // Limita o deltaHeight ao valor máximo permitido
    
    if (goingUp) {
        gateHeight += deltaHeight;
        ...
        gateHeight -= deltaHeight;
        if (gateHeight <= minHeight) {
            gateHeight = minHeight;
            goingUp = true;
        }
    }
}
...
glPushMatrix();
glTranslatef(-0.05, gateHeight, 11.593);
...
glPushMatrix();
glTranslatef(-0.6, gateHeight, -0.2);
...
void idle() {
    updateGate(); // atualiza a posição do portão
    glutPostRedisplay(); // solicita a redessinagem da janela
}
int main(int argc, char** argv){
  ...
  glutCreateWindow ("Portaria Condom?nio");
  init ();
    glutIdleFunc(idle); // registra a função idle()
